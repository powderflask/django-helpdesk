...
from helpdesk.models import (
    Ticket, Queue, Milestone, FollowUp, TicketChange, PreSetReply, Attachment, SavedSearch,
    ...
    if request.GET.get('saved_query', None):
        ...
        try:
            ...
            return HttpResponseRedirect(reverse('helpdesk:list'))
              ...
              'milestone' in request.GET or
              'assigned_to' in request.GET or
              'ticket_type' in request.GET or
        ...
        if sort not in ('ticket_type', 'status', 'assigned_to', 'created', 'modified', 'title', 'queue', 'milestone', 'priority'):
            sort = 'created'
...
def create_ticket(request):
    ...
    if request.method == 'POST':
        ...
        form.fields['milestone'].choices = [('', '--------')] + [
            [m.id, m.title] for m in Milestone.objects.filter(is_active=True)]
